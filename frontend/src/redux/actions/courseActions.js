...
export const ListcoursesbyTopic = (Topic,Allcourses = false) => async (dispatch) =>{
    try {
        dispatch({type : LIST_COURSES_REQUEST })
        const {data} = await axios.get(`/courses/topic/?Topic=${Topic}&All=${Allcourses}`)
...
export const ListnewCourses = (Topic,All= false) => async (dispatch) =>{
    try {
        dispatch({type : LIST_NEW_COURSES_REQUEST })
        const {data} = await axios.get(`/courses/topic/?Topic=${Topic}&All=${All}&New=true`)
        dispatch({type : LIST_NEW_COURSES_SUCCESS , payload : data})
        console.log(data)
        } catch (error) {
            dispatch({type : LIST_NEW_COURSES_FAIL , 
                      payload : error.response && error.response.data.message 
                      ? error.response.data.message : error.message, })
...
export const Listcoursesbypobularity = (Topic = '') => async (dispatch) =>{
    try {
